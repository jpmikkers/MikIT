...
MDRIVER_MIXER::MDRIVER_MIXER() : MDRIVER()
{
...
MDRIVER_MIXER::~MDRIVER_MIXER()
{
}



void MDRIVER_MIXER::Init()
{
...
void MDRIVER_MIXER::Exit()
{
}



void MDRIVER_MIXER::Start()
{
    int t;
...
*/  }
    ...
    maxvol=amplifytable[channels-1]<<1;
...
void MDRIVER_MIXER::Stop()
{
}



void MDRIVER_MIXER::SetBPM(UBYTE ibpm)
{
    bpm=ibpm;
}
...
void MDRIVER_MIXER::FreeVoice(int handle)
{
    if(handle<0) return;
...
int MDRIVER_MIXER::SampleLoad(MINPUT *i,ULONG length,ULONG reppos,ULONG repend,UWORD flags)
{
    int handle;
    ULONG t;

//  printf("Loading a %d sample sample\n",length);
...
void MDRIVER_MIXER::SampleFree(int handle)
{
    if(handle>=0){
...
SLONG MDRIVER_MIXER::VOICEPARMS::fraction2long(ULONG dividend,UWORD divisor)
/*
...
int MDRIVER_MIXER::samples2bytes(int v)
{
...
int MDRIVER_MIXER::bytes2samples(int v)
{
    if(mode & DMODE_16BITS) v>>=1;
    if(mode & DMODE_STEREO) v>>=1;
    return v;
...
void MDRIVER_MIXER::Mix16StereoNormal(SWORD *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG lvolmul,SLONG rvolmul)
{
    SWORD sample;
...
void MDRIVER_MIXER::Mix16MonoNormal(SWORD *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG volmul)
{
    SWORD sample;
...
void MDRIVER_MIXER::Mix16StereoInterp(SWORD *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG lvolmul,SLONG rvolmul)
{
    int a,b,s;
...
void MDRIVER_MIXER::Mix16MonoInterp(SWORD *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG volmul)
{
    int a,b,s;
...
void MDRIVER_MIXER::Mix08StereoNormal(SBYTE *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG lvolmul,SLONG rvolmul)
{
    SBYTE sample;
...
void MDRIVER_MIXER::Mix08MonoNormal(SBYTE *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG volmul)
{
    SBYTE sample;
...
void MDRIVER_MIXER::Mix08StereoInterp(SBYTE *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG lvolmul,SLONG rvolmul)
{
    int a,b,s;
...
void MDRIVER_MIXER::Mix08MonoInterp(SBYTE *srce,SLONG *dest,SLONG index,SLONG increment,UWORD todo,SLONG volmul)
{
    int a,b,s;
...
UWORD MDRIVER_MIXER::NewPredict(SLONG index,SLONG end,SLONG increment,UWORD todo)
/*
...
void MDRIVER_MIXER::Sample32To8Copy(SLONG *srce,SBYTE *dest,ULONG count,UBYTE shift)
{
	SLONG c;
...
void MDRIVER_MIXER::Sample32To16Copy(SLONG *srce,SWORD *dest,ULONG count,UBYTE shift)
{
	SLONG c;
...
void MDRIVER_MIXER::AddChannel(VOICEPARMS *vnf,UWORD todo)
/*
...
void MDRIVER_MIXER::WritePortion(SBYTE *buf,int todo)
/*
...
void MDRIVER_MIXER::VOICEPARMS::NewParms()
{
...
void MDRIVER_MIXER::WriteSamples(SBYTE *buffer,int todo)
{
	int t,part;
	...
	while(todo>0){
		...
		if(tickleft==0){
			...
			for(t=0;t<channels;t++)	channel[t].NewParms();
		}
...
int MDRIVER_MIXER::WriteBytes(SBYTE *buffer,int todo)
{
...
void MDRIVER_MIXER::WriteSilentSamples(SBYTE *buffer,int todo)
{
}


int MDRIVER_MIXER::WriteSilentBytes(SBYTE *buffer,int todo)
{
	if(mode&DMODE_16BITS){
...
void MDRIVER_MIXER::VoiceSetVolume(int voice,UBYTE volume)
{	
	if(voice<0) return;
...
void MDRIVER_MIXER::VoiceSetPanning(int voice,UBYTE panning)
{	
	if(voice<0) return;
...
void MDRIVER_MIXER::VoiceSetFrequency(int voice,ULONG frequency)
{	
	if(voice<0) return;
...
void MDRIVER_MIXER::VoiceStop(int voice)
{	
	if(voice<0) return;
...
int MDRIVER_MIXER::VoiceActive(int voice)
{	
	if(voice<0) return 0;
...
void MDRIVER_MIXER::VoiceKeyOff(int voice)
{
...
void MDRIVER_MIXER::VoiceKeyOn(int voice)
{
	if(!channel[voice].keyon){
...
void MDRIVER_MIXER::VoicePlay(int voice,int handle,ULONG start,ULONG size,ULONG reppos,ULONG repend,ULONG sreppos,ULONG srepend,UWORD flags)
{
	if(voice<0) return;
	if(handle<0) return;
